...
- `get_macros` escalates through abridged, full, and unfiltered nutrient pulls (plus `labelNutrients`) and now throws a descriptive error when a Foundation entry still omits any macro so MCP clients do not accept partial data silently. Anytime the guard or retry order changes, update README troubleshooting guidance and note the conversion formula (fat × 9 + protein/carbs × 4) for manual derivations.
